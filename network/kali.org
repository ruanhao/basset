...
#+begin_src sh
  ...
  hcxpcapngtool -o output.hc22000 input.cap # 如果有握手信息，会生成 output.hc22000
#+end_src

*** 掩码暴力破解

#+begin_src sh
  hashcat -m 22000 -a 3 output.hc22000 -1 ?l?d ?1?1?1?1?1?1?1?1 --session mywifi --force # 8 位密码，小写字母与数字组合
  hashcat --restore --session mywifi # 用于恢复计算，可以手动 checkpoint
...
#+begin_src
  ...
  hashcat -m 22000 output.hc22000 /dir/to/wordlist/
#+end_src
...
* 常用链接
  ...
  - [[https://hashcat.net/wiki/doku.php?id=example_hashes][hashcat hash types]]
