...
[[https://www.rabbitmq.com/logging.html][RabbitMQ Logging]]
...
First choose one partition which you trust the most. Then stop all nodes in the other partitions and spin them all up again.

**** 全部重启

It may be simpler to stop the whole cluster and start it again;
if so make sure that the first node you start is from the trusted partition.


** HA

*** [[https://www.rabbitmq.com/ha.html#start-stop][Stopping Nodes and Synchronisation]]

When a mirror rejoins a mirrored queue, it throws away any durable local contents it already has and starts empty.
Its behaviour is at this point the same as if it were a [[https://www.rabbitmq.com/ha.html#unsynchronised-mirrors][new node joining the cluster]].
...
[[file:img/rabbit_federation_exchange.png]]
